...
def get_gaussian_kernel(img, kernel_size=3, sigma=2, channels=1):
    ...
    # gaussian_filter = F.Conv2d(in_channels=channels, out_channels=channels, kernel_size=kernel_size, groups=channels,
    #                             bias=False, padding=kernel_size // 2)

    # gaussian_filter.weight.data = gaussian_kernel
    # gaussian_filter.weight.requires_grad = False

    res = F.conv2d(img, gaussian_kernel, bias, padding=kernel_size // 2)
    return res
...
class Detail_Loss(nn.Module):
    def __init__(self):
        ...
        # self.gua3 = get_gaussian_kernel(kernel_size=3)
        # self.gua5 = get_gaussian_kernel(kernel_size=5)
        # self.gua7 = get_gaussian_kernel(kernel_size=7)

    def forward(self, Tensor_Fus, Tensor_Inf):
        LB = functional_conv2d(Tensor_Inf)
        b = get_gaussian_kernel(kernel_size=3, img=LB)
        bb = get_gaussian_kernel(kernel_size=5, img=LB)
        bbb = get_gaussian_kernel(kernel_size=7, img=LB)

        loss = torch.mean(torch.abs(Tensor_Fus - Tensor_Inf) * (b + bb + bbb))
        return loss

class UDGANFUSEIVModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'G_L1', 'G_SWIN', 'G_Clarity', 'G_TV', 'G_SSIM', 'DI_real', 'DV_fake','G_Detail']
    ...
    def backward_G(self):
        ...
        self.loss_G_L1 = self.criterionL1(self.fake_F, self.real_A) * 100
        # self.loss_G_L1 = 0.5 * (self.loss_G_L1A + self.loss_G_L1B) * self.opt.lambda_L1
        self.loss_G_Detail1 = self.DetailLoss(self.fake1, self.real_A)
        self.loss_G_Detail2 = self.DetailLoss(self.fake2, self.real_A)
        self.loss_G_Detail3 = self.DetailLoss(self.fake3, self.real_A)
        self.loss_G_DetailF = self.DetailLoss(self.fake_F, self.real_A)
        self.loss_G_Detail = (self.loss_G_Detail1 + self.loss_G_Detail2 + self.loss_G_Detail3 + self.loss_G_DetailF) * 0.25 * 200

        #self.loss_G_L2 = self.criterionL2(self.fake_F, self.real_A)

        self.loss_G_Clarity1 = self.Clarity(self.real_B, self.fake1)  #100
        self.loss_G_Clarity2 = self.Clarity(self.real_B, self.fake2)
        self.loss_G_Clarity3 = self.Clarity(self.real_B, self.fake3)
        self.loss_G_ClarityF = self.Clarity(self.real_B, self.fake_F)
        self.loss_G_Clarity = (self.loss_G_Clarity1 + self.loss_G_Clarity2 + self.loss_G_Clarity3 + self.loss_G_ClarityF) * 0.25 * 100
        self.loss_G_TV1 = (self.criterionL1(self.TVloss(self.fake1), self.TVloss(self.real_A)) + self.criterionL1(self.TVloss(self.fake1), self.TVloss(self.real_B))) * 0.5
        self.loss_G_TV2 = (self.criterionL1(self.TVloss(self.fake2), self.TVloss(self.real_A)) + self.criterionL1(self.TVloss(self.fake2), self.TVloss(self.real_B))) * 0.5
        self.loss_G_TV3 = (self.criterionL1(self.TVloss(self.fake3), self.TVloss(self.real_A)) + self.criterionL1(self.TVloss(self.fake3), self.TVloss(self.real_B))) * 0.5
        self.loss_G_TVF = (self.criterionL1(self.TVloss(self.fake_F), self.TVloss(self.real_A)) + self.criterionL1(self.TVloss(self.fake_F), self.TVloss(self.real_B))) * 0.5
        self.loss_G_TV = (self.loss_G_TV1 + self.loss_G_TV2 + self.loss_G_TV3 + self.loss_G_TVF) * 0.25 * 200
        self.loss_G_SSIMB = 1 - self.SSIM(self.fake_F, self.real_B)
        self.loss_G_SSIMA = 1 - self.SSIM(self.fake_F, self.real_A)
        self.loss_G_SSIM = (self.loss_G_SSIMA + self.loss_G_SSIMB) * 0.5 * 10 * 0
        ...
        self.loss_G_SWIN1 = [self.criterionL1(self.feature1[i], 0.5 * (self.feature_A[i] + self.feature_B[i] + torch.abs(self.feature_A[i] - self.feature_B[i]))) for i in range(4)]
        self.loss_G_SWIN2 = [self.criterionL1(self.feature2[i], 0.5 * (self.feature_A[i] + self.feature_B[i] + torch.abs(self.feature_A[i] - self.feature_B[i]))) for i in range(4)]
        self.loss_G_SWIN3 = [self.criterionL1(self.feature3[i], 0.5 * (self.feature_A[i] + self.feature_B[i] + torch.abs(self.feature_A[i] - self.feature_B[i]))) for i in range(4)]
        self.loss_G_SWINF = [self.criterionL1(self.feature_F[i], 0.5 * (self.feature_A[i] + self.feature_B[i] + torch.abs(self.feature_A[i] - self.feature_B[i]))) for i in range(4)]
        ...
        self.loss_G_SWIN = (sum(self.loss_G_SWIN1) + sum(self.loss_G_SWIN2) + sum(self.loss_G_SWIN3) + sum(self.loss_G_SWINF)) * 0.25 * 20



        # combine loss and calculate gradients
        self.loss_G = self.loss_G_GAN + self.loss_G_L1 + self.loss_G_Clarity + self.loss_G_SSIM + self.loss_G_TV + self.loss_G_SWIN + self.loss_G_Detail
