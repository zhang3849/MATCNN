...
class MEDFUSEModel(BaseModel):
    ...
    def backward_G(self):
        ...
        self.loss_GI_GAN = self.criterionGAN(pred_fakeI, True)
        ...
        self.loss_G_L1 = self.criterionL1(self.fake_F, 0.5 * (self.real_A + self.real_B + abs(self.real_A - self.real_B))) * self.opt.lambda_L1
        ...
        self.loss_G_Clarity = self.Clarity(self.real_A, self.real_B, self.fake_F) * 100  #100
        self.loss_G_TVA = self.criterionL1(self.TVloss(self.fake_F), self.TVloss(self.real_A))
        self.loss_G_TVB = self.criterionL1(self.TVloss(self.fake_F), self.TVloss(self.real_B))
        self.loss_G_TV = (self.loss_G_TVA + self.loss_G_TVB) * 0.5 * 20
        self.loss_G_SSIMB = 1 - self.SSIM(self.fake_F, self.real_B)
        self.loss_G_SSIMA = 1 - self.SSIM(self.fake_F, self.real_A)
        self.loss_G_SSIM = (self.loss_G_SSIMA + self.loss_G_SSIMB) * 0.5 * 5 * 0
        ...
        self.loss_G_SWINa = [self.criterionL1(self.feature_F[i], 0.5 * (self.feature_A[i] + self.feature_B[i] + torch.abs(self.feature_A[i] - self.feature_B[i]))) for i in range(4)]
        ...
        self.loss_G_SWIN = sum(self.loss_G_SWINa) * 10
